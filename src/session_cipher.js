...
function assertBuffer(value) {
    if (!(value instanceof Buffer)) {
        throw new TypeError(
            `Expected Buffer instead of: ${value.constructor.name}`
        );
    }
    return value;
}
...
class SessionCipher {
    ...
    async queueJob(awaitable) {
        return queueJob(this.addr.toString(), awaitable).catch(err => {
            ...
            return await this.queueJob(async () => {
                ...
                if (session.pendingPreKey) {
                    ...
                    const preKeyMsg = protobufs.PreKeyWhisperMessage.create({
                        ...
                        registrationId:
                            await this.storage.getOurRegistrationId(),
                    ...
                    body = Buffer.concat([
                        ...
                        Buffer.from(
                            protobufs.PreKeyWhisperMessage.encode(
                                preKeyMsg
                            ).finish()
                        )
                    ]);
    ...
    async decryptWithSessions(data, sessions) {
        ...
        errs.forEach(e => console.error(`Session error: ${e.message}`));
    ...
    async decryptWhisperMessage(data) {
        try {
            ...
            return await this.queueJob(async () => {
                ...
                const result = await this.decryptWithSessions(
                    data,
                    record.getSessions()
                );
    ...
    async decryptPreKeyWhisperMessage(data) {
        try {
            ...
            return await this.queueJob(async () => {
                ...
                const preKeyProto = protobufs.PreKeyWhisperMessage.decode(
                    data.slice(1)
                );
                if (!record) {
                ...
                const preKeyId = await builder.initIncoming(
                    record,
                    preKeyProto
                );
            ...
            console.error(`PreKey decryption error: ${err.message}`);
            return null;
        }
    }
