...
class SessionBuilder {
    ...
    async initOutgoing(device) {
        ...
        return await queueJob(fqAddr, async () => {
            try {
                ...
                console.error(`Failed to initialize outgoing session for ${fqAddr}:`, error.message);
            }
        });
    }
    ...
    async initIncoming(record, message) {
        ...
        try {
            ...
            console.error(`Failed to initialize incoming session for ${fqAddr}:`, error.message);
        }
    }
    ...
    calculateSendingRatchet(session, remoteKey) {
        try {
            ...
            console.error("Sending ratchet calculation error:", error.message);
        }
    }
}
